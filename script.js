...
const GakuseiDataService = (() => {
  ...
  async function getAcademicData(inputId) {
    try {
      ...
      const academicRecords = await getAcademicRecordData(
        master.studentId,
        dorm.code
      );
...
const AUTO_REFRESH_MS=60000;let currentStudentId='',refreshTimer=null,pdfBusy=false,promotionData=null,currentTheme='front',currentDocumentReview=null,academicLoadToken=0,currentAcademicReady=false;
    ...
    window.addEventListener('load',()=>{renderDecorations('front');$('idInput').addEventListener('keydown',e=>{if(e.key==='Enter')searchGakusei()});$('documentReviewModal').addEventListener('click',e=>{if(e.target===$('documentReviewModal'))closeDocumentReview()});window.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('documentReviewModal').classList.contains('hidden'))closeDocumentReview()});const q=new URLSearchParams(location.search);const initial=q.get('id')||sessionStorage.getItem('lastGakuseiId')||'';if(initial){$('idInput').value=initial;searchGakusei()}});
    ...
    async function fetchStudent(id,silent){
      ...
      try{
         ...
         */
        loadAcademicRecordsAsync(response.data.nomorId,requestToken,silent);
      }catch(error){
    ...
    function showAcademicLoading(){
      ...
      $('ledgerButton').textContent='LOADING LEDGER...';
    ...
    function markAcademicRefreshing(){
      ...
      $('ledgerButton').disabled=true;
      $('ledgerButton').textContent='SYNCING...';
    }
    ...
    async function requestPdf(mode,sheetName,button){
      ...
      showPdf(normalizedMode==='ledger'?'Loading complete academic ledger...':'Loading academic record...');
      ...
      try{
        ...
        setStatus(
          normalizedMode==='ledger'
            ? 'Complete academic ledger is ready in the review window.'
    ...
    async function buildAcademicPdf(payload){
      ...
      const isLedger=copy.mode==='ledger';
      await openDocumentReview({
        title:isLedger?'ACADEMIC LEDGER':'STUDENT ACADEMIC RECORD',
        subtitle:isLedger
    ...
    function createAcademicRecordPage(payload,record,options){
      ...
      identity.appendChild(photoWrap);

      const identityDetails=document.createElement('div');
      identityDetails.className='pdfIdentityDetails';
      ...
      house.className='pdfHouse';
      const houseLogo=document.createElement('div');
      houseLogo.className='pdfHouseLogo';
      appendDocumentImage(
        houseLogo,
        dormLogoUrls,
        'pdfHouseLogoImage',
        (s.asrama&&s.asrama.name?s.asrama.name:'House')+' logo',
        'HOUSE'
      );
      house.appendChild(houseLogo);

      const houseText=document.createElement('div');
      houseText.className='pdfHouseText';
      houseText.innerHTML=
        '<div class="idLabel">HOUSE</div>'+ 
        '<div class="pdfHouseName">'+escapeHtml(s.asrama&&s.asrama.name||'-')+'</div>';
      house.appendChild(houseText);

      const shownGrade=document.createElement('div');
      shownGrade.className='pdfShownGrade';
      shownGrade.innerHTML=
        '<div class="pdfShownGradeLabel">SHOWN GRADE</div>'+ 
        '<div class="pdfShownGradeValue">'+escapeHtml(record.nenseiLabel||'-')+'</div>';
      house.appendChild(shownGrade);

      identityDetails.appendChild(house);
      identity.appendChild(identityDetails);
