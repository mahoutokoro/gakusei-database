...
function academicRecordStorageKey(studentId) {
  return `mahoutokoro-academic-records-v14-v18-engine-${CONFIG.ACADEMIC_SPREADSHEET_ID}-${normalizeId(studentId)}`;
}
...
function restoreFailedAcademicRecords(studentId, records, failedSheetNames) {
  ...
  async function readAcademicSemesterRecord(sheetName, studentId, dormCode, bypassCache = false) {
    ...
    const nensei = detectedNensei || detectTaNenseiFromAcademicBlock(display, selectedIndex);
...
function detectTaNenseiFromAcademicBlock(displayValues, studentRowIndex) {
  const firstRow = Math.max(0, studentRowIndex - 28);
  for (let rowIndex = studentRowIndex; rowIndex >= firstRow; rowIndex--) {
    ...
    if (
      /(?:^| )TA(?: |$)/.test(text) ||
      /(?:^| )TUGAS AKHIR(?: |$)/.test(text) ||
      /(?:^| )FINAL PROJECT(?: |$)/.test(text) ||
      /(?:^| )THESIS(?: |$)/.test(text)
    ) {
      return 7;
    }
...
const AUTO_REFRESH_MS=60000;let currentStudentId='',refreshTimer=null,gpRankingTimer=null,gpRankingLoadToken=0,pdfBusy=false,promotionData=null,nenseiData=null,currentTheme='front',currentDocumentReview=null,academicLoadToken=0,currentAcademicReady=false,academicLoadPromise=null,browserNativeCaptureSession=null;
    ...
    window.addEventListener('load',()=>{
      ...
      const initial=q.get('id')||sessionStorage.getItem('lastGakuseiId')||'';
      if(initial){
    ...
    function goHome(){
      ...
      sessionStorage.removeItem('lastGakuseiId');
...
function startGpRankingRefresh(){
    ...
    function renderStudent(response,silent){
      ...
      if(!response||!response.success){
        setStatus(response&&response.message?response.message:'Student not found.');
        return false;
      }
      ...
      if(d.isGraduated){
        renderLifeAfterGraduation(
          d.postGraduationId||'',
          Boolean(d.postGraduationResidentActive)
        );
      }else{
      ...
      sessionStorage.setItem('lastGakuseiId',currentStudentId);

      const url=new URL(location.href);
      url.searchParams.set('id',currentStudentId);
      history.replaceState({},'',url);
...
function appendAcademicPageWatermark(page,logoUrls){
  ...
  const tryNext=()=>{
    ...
    img.src=cache(candidates[index++]);
  };
  img.onload=()=>{
...
function makeTransparentStampDataUrl(image){
  try{
    ...
    function closeDocumentReview(){
      ...
      document.body.classList.remove('printing-document-review');
      $('documentReviewPages').innerHTML='';
      if(browserNativeCaptureSession){
        try{browserNativeCaptureSession.stop();}catch(error){}
        browserNativeCaptureSession=null;
      }
    ...
    async function createBrowserNativeCaptureSession(){
      if(!navigator.mediaDevices||typeof navigator.mediaDevices.getDisplayMedia!=='function'){
        throw new Error('Browser-native capture is unavailable. Open this site in a current Chromium browser over HTTPS.');
      }
      const stream=await navigator.mediaDevices.getDisplayMedia({
        video:{displaySurface:'browser',frameRate:{ideal:8,max:12}},
        audio:false,
      ...
      if(!track){
        ...
        throw new Error('No browser capture video track was returned.');
      }
      const settings=typeof track.getSettings==='function'?track.getSettings():{};
      if(settings.displaySurface&&settings.displaySurface!=='browser'){
        stream.getTracks().forEach(item=>item.stop());
        throw new Error('For an exact export, choose THIS TAB / the Mahoutokoro browser tab in the share dialog.');
      }
      try{
        if(typeof track.applyConstraints==='function'){
          await track.applyConstraints({frameRate:{ideal:5,max:8},width:{ideal:7680},height:{ideal:4320}});
        }
      }catch(error){}
      ...
      await new Promise((resolve,reject)=>{
        const timeout=setTimeout(()=>reject(new Error('Browser capture stream did not become ready.')),10000);
        const ready=()=>{
          if(video.videoWidth&&video.videoHeight){clearTimeout(timeout);resolve();}
        };
        video.addEventListener('loadedmetadata',ready,{once:true});
        video.addEventListener('canplay',ready,{once:true});
        video.addEventListener('error',()=>{clearTimeout(timeout);reject(new Error('Browser capture video could not be initialized.'));},{once:true});
      });
      ...
      return {
        stream,track,video,
        stop(){
          try{stream.getTracks().forEach(item=>item.stop());}catch(error){}
    ...
    async function getReusableBrowserNativeCaptureSession(){
      if(
        browserNativeCaptureSession &&
        browserNativeCaptureSession.track &&
        browserNativeCaptureSession.track.readyState==='live' &&
        browserNativeCaptureSession.video
      ){
        return browserNativeCaptureSession;
      }

      browserNativeCaptureSession=await createBrowserNativeCaptureSession();
      const activeSession=browserNativeCaptureSession;

      if(activeSession.track&&typeof activeSession.track.addEventListener==='function'){
        activeSession.track.addEventListener('ended',()=>{
          if(browserNativeCaptureSession===activeSession){
            browserNativeCaptureSession=null;
          }
        },{once:true});
      }
    ...
    async function waitForBrowserNativeFrame(session){
      await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
      const waitOneFrame=()=>new Promise(resolve=>{
        let finished=false;
        const done=()=>{if(finished)return;finished=true;resolve();};
        const timeout=setTimeout(done,220);
        if(session.video&&typeof session.video.requestVideoFrameCallback==='function'){
          session.video.requestVideoFrameCallback(()=>{clearTimeout(timeout);done();});
        }
      });
      await waitOneFrame();
      await waitOneFrame();
    ...
    async function createExactDocumentCapture(pageElement,scale,captureSession){
      const desiredScale=Math.max(1,Number(scale)||3);
      if(!pageElement)throw new Error('Document preview page is unavailable.');
      if(!captureSession||!captureSession.video)throw new Error('Browser-native capture session is unavailable.');
      await ensureJapaneseDocumentFont();
      await waitReviewImages(pageElement,25000);
      if(document.fonts&&document.fonts.ready){try{await document.fonts.ready;}catch(error){}}

      const originalParent=pageElement.parentNode;
      const originalNextSibling=pageElement.nextSibling;
      const originalStyle=pageElement.getAttribute('style');
      if(!originalParent)throw new Error('Document preview page is detached.');

      const overlay=document.createElement('div');
      overlay.className='browserNativeCaptureOverlay';
      overlay.setAttribute('aria-hidden','true');
      const viewport=document.createElement('div');
      viewport.className='documentReviewViewport browserNativeCaptureViewport';
      const pages=document.createElement('div');
      pages.className='documentReviewPages browserNativeCapturePages';
      const stage=document.createElement('div');
      stage.className='browserNativeCaptureStage';
      pages.appendChild(stage);
      viewport.appendChild(pages);
      overlay.appendChild(viewport);
      document.body.appendChild(overlay);
      stage.appendChild(pageElement);
      ...
      pageElement.style.height='1344px';
      pageElement.style.minWidth='816px';
      pageElement.style.maxWidth='816px';
      pageElement.style.minHeight='1344px';
      pageElement.style.maxHeight='1344px';
      ...
      try{
        await waitForBrowserNativeFrame(captureSession);
        ...
        if(!videoWidth||!videoHeight)throw new Error('The Chromium capture stream has no usable resolution.');
        ...
        const streamScaleX=videoWidth/viewportWidth;
        const streamScaleY=videoHeight/viewportHeight;
        const baseStreamScale=Math.max(.1,Math.min(streamScaleX,streamScaleY));
        const renderScale=desiredScale/baseStreamScale;
        stage.style.transformOrigin='0 0';
        stage.style.transform='scale('+renderScale+')';
        stage.style.width='816px';
        stage.style.height='1344px';

        const rawWidth=Math.max(1,Math.ceil(816*renderScale*streamScaleX));
        const rawHeight=Math.max(1,Math.ceil(1344*renderScale*streamScaleY));
        const rawCanvas=document.createElement('canvas');
        rawCanvas.width=rawWidth;
        rawCanvas.height=rawHeight;
        const rawContext=rawCanvas.getContext('2d');
        if(!rawContext)throw new Error('Unable to create the browser capture canvas.');
        rawContext.imageSmoothingEnabled=true;
        rawContext.imageSmoothingQuality='high';

        for(let y=0;y<rawHeight;y+=videoHeight){
          for(let x=0;x<rawWidth;x+=videoWidth){
            stage.style.left=(-x/streamScaleX)+'px';
            stage.style.top=(-y/streamScaleY)+'px';
            await waitForBrowserNativeFrame(captureSession);
            const copyWidth=Math.min(videoWidth,rawWidth-x);
            const copyHeight=Math.min(videoHeight,rawHeight-y);
            rawContext.drawImage(video,0,0,copyWidth,copyHeight,x,y,copyWidth,copyHeight);
          }
        }

        const targetWidth=Math.max(1,Math.round(816*desiredScale));
        const targetHeight=Math.max(1,Math.round(1344*desiredScale));
        if(rawWidth===targetWidth&&rawHeight===targetHeight)return rawCanvas;
        const finalCanvas=document.createElement('canvas');
        finalCanvas.width=targetWidth;
        finalCanvas.height=targetHeight;
        const finalContext=finalCanvas.getContext('2d');
        if(!finalContext)throw new Error('Unable to create the final HD capture canvas.');
        finalContext.imageSmoothingEnabled=true;
        finalContext.imageSmoothingQuality='high';
        finalContext.drawImage(rawCanvas,0,0,rawWidth,rawHeight,0,0,targetWidth,targetHeight);
        rawCanvas.width=1;
        rawCanvas.height=1;
        return finalCanvas;
      }finally{
        if(originalNextSibling&&originalNextSibling.parentNode===originalParent){
          originalParent.insertBefore(pageElement,originalNextSibling);
        }else{
          originalParent.appendChild(pageElement);
        }
        if(originalStyle===null)pageElement.removeAttribute('style');
        else pageElement.setAttribute('style',originalStyle);
        overlay.remove();
      }
    }
...
async function downloadReviewedDocumentAsPdf(){
      ...
      showPdf(
        browserNativeCaptureSession&&browserNativeCaptureSession.track&&browserNativeCaptureSession.track.readyState==='live'
          ? 'Preparing ultra-HD export from the active Chromium capture session...'
          : 'Chrome requires one tab-capture permission for exact browser-rendered export. Choose THIS TAB / the current Mahoutokoro tab. This permission is reused until the preview is closed.'
      );
      let nativeCaptureSession=null;

      try{
        nativeCaptureSession=await getReusableBrowserNativeCaptureSession();
        ...
        const pages=Array.from(
          $('documentReviewPages').querySelectorAll('.pdfPage')
        );
        if(!pages.length)throw new Error('No document page is available.');

        const previewWidth=816;
        const previewHeight=1344;
        const pdfWidth=612;
        ...
        for(let index=0;index<pages.length;index++){
          ...
          showPdf('Matching preview page '+(index+1)+' of '+pages.length+'...');

          const canvas=await createExactDocumentCapture(pages[index],5,nativeCaptureSession);
        ...
        pdf.save(fileName);
        setStatus('PDF downloaded from Chromium native preview capture.');
      }catch(error){
        console.error(error);
    ...
    async function downloadReviewedDocumentAsPng(){
      ...
      showPdf(
        browserNativeCaptureSession&&browserNativeCaptureSession.track&&browserNativeCaptureSession.track.readyState==='live'
          ? 'Preparing ultra-HD PNG from the active Chromium capture session...'
          : 'Chrome requires one tab-capture permission for exact browser-rendered export. Choose THIS TAB / the current Mahoutokoro tab. This permission is reused until the preview is closed.'
      );
      let nativeCaptureSession=null;
      ...
      try{
        ...
        if(pages.length===1){
          ...
          const canvas=await createExactDocumentCapture(pages[0],6,nativeCaptureSession);

          const blob=await canvasToBlob(canvas,'image/png');
          triggerBlobDownload(blob,baseName+'.png');
          canvas.width=1;
          canvas.height=1;
          setStatus('HD PNG downloaded from Chromium native preview capture.');
        }else{
          ...
          for(let index=0;index<pages.length;index++){
            ...
            const canvas=await createExactDocumentCapture(pages[index],6,nativeCaptureSession);
          ...
          setStatus(pages.length+' Chromium-native HD PNG pages downloaded as ZIP successfully.');
        }
      }catch(error){
        console.error(error);
